...
#define MAY_BE_HASH		2
	...
	do {
		...
		if (op == TAG_READ) {
			...
			r = dm_bufio_copyin(ic->bufio, tag, *metadata_block, *metadata_offset, *metadata_offset + to_copy);
			if (unlikely(r))
				return r;
		} else {
			/* e.g.: op == TAG_CMP */
		...
		__bio_for_each_segment(bv, bio, iter, dio->bio_details.bi_iter) {
			...
			do {
				...
				if (likely(checksums != checksums_onstack)) {
					...
					r = dm_integrity_rw_tag(ic, checksums, &dio->metadata_block, &dio->metadata_offset,
								ic->tag_size, dio->op == REQ_OP_READ ? TAG_CMP : TAG_WRITE);
					if (unlikely(r)) {
						kunmap_atomic(mem);
...
static int dm_integrity_ctr(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	unsigned journal_sectors, interleave_sectors, buffer_sectors, metadata_align, journal_watermark, sync_msec;
...
#define DIRECT_ARGUMENTS	4
	...
	while (extra_args--) {
		...
		if (sscanf(opt_string, "journal_sectors:%u%c", &val, &dummy) == 1)
			...
			buffer_sectors = val;
		else if (sscanf(opt_string, "metadata_align:%u%c", &val, &dummy) == 1) {
			if (val < 1 << SECTOR_SHIFT ||
			    (val & (val -1))) {
				ti->error = "Invalid metadata_align argument";
				goto bad;
			}
			metadata_align = val;
...
try_smaller_buffer:
	...
	ic->bufio = dm_bufio_client_create(ic->meta_dev ? ic->meta_dev->bdev : ic->dev->bdev,
			1U << (SECTOR_SHIFT + ic->log2_buffer_sectors), metadata_align, 1, 0,
			NULL, NULL);
	if (IS_ERR(ic->bufio)) {
		r = PTR_ERR(ic->bufio);
